...
def extract_data() -> pd.DataFrame:
    ...
    download_dataset = os.system("mkdir dataset;cd dataset;kaggle datasets download -d 'fayez/us-accidents'")
...
def write_gcs(parquet_path: Path) -> None:
    ...
    gcp_cloud_storage_bucket_block = GcsBucket.load("fayez-gcs")
    gcp_cloud_storage_bucket_block.upload_from_path(from_path=parquet_path, to_path=parquet_path),
    requests.timeout=6000,
    bucket._chunksize = 500_000
...
def write_bq(path: Path) -> None:
    ...
    gcp_credentials_block = GcpCredentials.load("fayez-gcs")
